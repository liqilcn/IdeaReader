...
import distributed
from models import data_loader_bertsumabs, model_builder_bertsumabs
from models.data_loader_bertsumabs import load_dataset
from models.loss import abs_loss
from models.model_builder_bertsumabs import AbsSummarizer
from models.predictor import build_predictor
from models.trainer_bertsumabs import build_trainer
...
def train_bertsumabs_multi(args):
...
def run(args, device_id, error_queue):
    ...
    try:
        ...
        train_bertsumabs_single(args, device_id)
...
def validate_bertsumabs(args, device_id):
    # 先用交叉熵loss选出5个最优模型，然后使用这个5个模型最后对测试集进行测试，生成最终的文件，选择最优的作为最后的模型
...
def validate(args, device_id, pt, step):  # 验证相应的checkpoint的performance
    ...
    valid_iter = data_loader_bertsumabs.Dataloader(args, load_dataset(args, 'valid', shuffle=False),
...
def test_bertsumabs(args, device_id, pt, step):
    device = "cpu" if args.visible_gpus == '-1' else "cuda"
    if (pt != ''):
        test_from = pt
    else:
    ...
    test_iter = data_loader_bertsumabs.Dataloader(args, load_dataset(args, 'test', shuffle=False),
...
def test_text_bertsumabs(args, device_id, pt, step):
    device = "cpu" if args.visible_gpus == '-1' else "cuda"
    if (pt != ''):
        test_from = pt
    else:
...
def train_bertsumabs(args, device_id):
    if (args.world_size > 1):
        train_bertsumabs_multi(args)
    else:
        train_bertsumabs_single(args, device_id)


def train_bertsumabs_single(args, device_id):
    ...
    if device_id >= 0:
        ...
        torch.cuda.set_device(device_id)  # 设置好device_id之后该进程的tensor或者模型通过.to(device)将会被转移到该device_id对应的显卡上
        torch.cuda.manual_seed(args.seed)

    if args.train_from != '':
        logger.info('Loading checkpoint from %s' % args.train_from)
        checkpoint = torch.load(args.train_from,
                                map_location=lambda storage, loc: storage)  # 将checkpoint加载到CPU上，lambda storage, loc: storage，这个lambda函数就是将参数加载到GPU上
    ...
    def train_iter_fct():
        return data_loader_bertsumabs.Dataloader(args, load_dataset(args, 'train', shuffle=True), args.batch_size, device,
                                      shuffle=True, is_test=False)

    model = AbsSummarizer(args, device, checkpoint, bert_from_extractive)
    if (args.sep_optim):  # BERT编码器是否与解码器分开优化
        optim_bert = model_builder_bertsumabs.build_optim_bert(args, model, checkpoint)
        optim_dec = model_builder_bertsumabs.build_optim_dec(args, model, checkpoint)
        optim = [optim_bert, optim_dec]
    else:
        optim = [model_builder_bertsumabs.build_optim(args, model, checkpoint)]
    ...
    # trainer是通用的，可以针对不同的模型，这样又提高了模型的通用性
