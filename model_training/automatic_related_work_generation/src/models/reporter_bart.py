...
class Statistics(object):
    ...
    * elapsed time
    ...
    def __init__(self, loss=0, grad_accum_count=0, n_correct=0):
        self.loss = loss
        self.grad_accum_count = grad_accum_count  # tgt的token个数，去除了起始符，保留了结束符，不包含[PAD]
        self.start_time = time.time()
    ...
    def update(self, stat, update_n_src_words=False):
        ...
        """
        self.loss += stat.loss
        self.grad_accum_count += stat.grad_accum_count

    def xent(self):
        """ compute cross entropy """
        return self.loss / self.grad_accum_count
    ...
    def output(self, step, num_steps, learning_rate, start):
        ...
        t = self.elapsed_time()
        logger.info(
            ("Step %2d/%5d; xent: %4.2f; lr: %7.8f; %6.0f sec")
            % (step, num_steps,
               self.xent(),
               learning_rate,
               time.time() - start))
