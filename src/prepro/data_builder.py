...
def format_to_lines(args):
    ...
    #         train_files.append(f)
        # else:
        #     train_files.append(f)
    # 这个参考dl_code/pointer_generator_network/dataset_gen_from_scisum.ipynb的生成方式，原始训练测试集到当前项目数据的映射
    # 先将delve数据集转换为cnn dailymail数据集的格式，然后使用处理该数据集的方法处理delve数据集
    # 以下3行是新增的

    #train_files = [pjoin(args.raw_path, f'{i+3000}.txt.json') for i in range(72927)]
    #valid_files = [pjoin(args.raw_path, f'{i+75927}.txt.json') for i in range(3000)]
    test_files = [pjoin(args.raw_path, f'{i}.txt.json') for i in range(0, 1)]
    # print(test_files)
