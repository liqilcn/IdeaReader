...
_sberts = {}
def _get_sbert(model_name, device) -> SentenceTransformer:
    global _sberts
    if model_name not in _sberts:
        _sberts[model_name] = SentenceTransformer(model_name, device=device)
    return _sberts[model_name]


def read(pubs: List[Publication], device: str, use_tfidf=True, use_sbert=True, use_prone=True, use_node2vec=False,
    ...
    if use_sbert:
        sbert = _get_sbert(model_name="../../models/torch/sentence_transformers/public.ukp.informatik.tu-darmstadt.de_reimers_sentence-transformers_v0.2_bert-base-nli-stsb-mean-tokens.zip", device=device)
