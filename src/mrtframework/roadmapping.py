...
from .base import Publication, Cluster, MrtType
...
def roadmap(pubs: List[Publication], mrtType: MrtType, n_clusters: int = 6, algorithm: str = KERNEL_KMEANS,
            alpha: float = 1.0, seed: Optional[int] = 42, beta: float = 1.0,
            supervision_links: List[Tuple[str, str]] = []) -> Tuple[List[Cluster], np.array]:
    set_seed(seed=seed)
    ...
    W = nx.to_numpy_array(build_graph_with_nodes_and_edges(nodes=nodelist, edges=supervision_links).to_undirected(),
                          nodelist=nodelist)
    ...
    for cluster in clusters.values():
        ...
        def build_timeline(pubs, mrtType):
        ...
        cluster.main_timeline = build_timeline(main_timeline_pubs, mrtType)
        cluster.secondary_timeline = build_timeline(secondary_timeline_pubs, mrtType)
        ...
        if len(cluster.secondary_timeline) > 0 and len(cluster.main_timeline) > 0:
            secondary_nearest = cluster.secondary_timeline[0]
            connector = cluster.main_timeline[0]
            for pub in reversed(cluster.main_timeline):
                if mrtType == MrtType.UP and pub.year >= secondary_nearest.year:
                    connector = pub
                    break
                elif mrtType == MrtType.DOWN and pub.year <= secondary_nearest.year:
                    connector = pub
                    break
            cluster.graph.add_edge(connector.id, secondary_nearest.id)
