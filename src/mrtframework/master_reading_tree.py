...
class MasterReadingTree:
    ...
    def __init__(self, provider: DataProvider, query_pub: Publication, mrtType: MrtType, device: str, expected_size: int = 100,
        ...
        self.pubs = retrieve(provider=provider, query_pub=query_pub, mrtType=mrtType, expected_size=expected_size)
        logger.info('MRT is reading')
        self.embeddings = read(self.pubs, device=device, use_tfidf=use_tfidf, use_sbert=use_sbert, use_prone=use_prone,
                               use_node2vec=use_node2vec,
                               tfidf_ngram_begin=tfidf_ngram_begin, tfidf_ngram_end=tfidf_ngram_end,
                               tfidf_max_features=tfidf_max_features, sbert_model_name=sbert_model_name,
                               prone_features=prone_features, node2vec_features=node2vec_features)
        logger.info('MRT is roadmapping')
        self.clusters, self.kernel = roadmap(self.pubs, mrtType, n_clusters=n_clusters, algorithm=algorithm,
                                             alpha=alpha, seed=seed, beta=beta, supervision_links=supervision_links)
    ...
    def to_json(self):
        ...
        return {
            ...
            'branches': [[[pub2json(pub) for pub in cluster.main_timeline],
                          [pub2json(pub) for pub in cluster.secondary_timeline]] for cluster in clusters],
